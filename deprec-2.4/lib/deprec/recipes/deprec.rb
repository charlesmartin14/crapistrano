...
# chm 3 dec 2011    adding nosql
#                   changed default ruby, db
...
Capistrano::Configuration.instance(:must_exist).load do 
  ...
  CHOICES_APPSERVER = [:passenger, :none] # unicorn, any colour you like guys
  CHOICES_DATABASE  = [:mysql, :postgresql, :sqlite, :none]  
  CHOICES_NOSQL  = [:redis, :mongodb, :memcache, :none]   
  CHOICES_SEARCH  = [:xapian, :ferret, :sphynx]  # TODO:  implement at least xapian, others dont matter yet   
  ...
  set :db_server_type,  :none  # default no longer mysql
  set :nosql_type,  :redis
 
  # TODO:  only prompt if unknown
...
#  set :run_method, :sudo 
  ...
  set :apps_root,    '/home/ubuntu'  # chm  29-aug-2012  can not over-write deploy_to inside capfile !!!
  ...
  set :current_user, `whoami`.chomp    # is this local or remote?  probably local?
