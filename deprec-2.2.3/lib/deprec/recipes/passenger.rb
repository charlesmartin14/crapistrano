...
Capistrano::Configuration.instance(:must_exist).load do 
  namespace :deprec do 
    namespace :passenger do
      ...
      #set(:passenger_document_root) { "#{current_path}" }  #/public not used?  # TODO:  this is crazy
      set(:passenger_document_root) { "/var/www" } 
      ...
      task :install, :roles => :app do
        #TODO:  mkdir,  sudo chown -R ubuntu ~/app/passenger/
        # these directories are not created
        install_deps
      ...
      PROJECT_CONFIG_FILES[:passenger] = [
        ...
        { :template => 'apache_vhost.erb',
          :path => "#{deploy_to}/passenger/apache_vhost",
          :mode => 0755,
          :owner => 'root:root'},
          
        {:template => 'logrotate.conf.erb',
         :path => "#{deploy_to}/passenger/logrotate.conf", 
         :mode => 0644,
         :owner => 'root:root'}
      ...
      task :symlink_apache_vhost, :roles => :app do
        sudo "rm #{apache_vhost_dir}/000-default"
      ...
      task :activate_project, :roles => :app do
       # sudo "a2ensite #{application}"
        # 5 dec 2011
        #  hacking..SVN use_sudo does not work?
        top.deprec2.mkdir(passenger_document_root, :mode => 0775, :group => group_src, :via => :sudo)
        top.deprec.web.reload
      end
      ...
      task :restart, :roles => :app do
        run "#{sudo} touch #{current_path}/#{application}/tmp/restart.txt"
      end
